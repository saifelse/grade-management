...
public class GDataDownloader {
    ...
    private final static String FOLDER_FEED_URI = "https://docs.google.com/feeds/default/private/full/-/folder";
    private final static String SPREADSHEET_ENTRY_URI = "https://spreadsheets.google.com/feeds/spreadsheets/";
     ...
     * 
     * no longer deprecated (Google Docs folders don't seem to work correctly "http://code.google.com/p/gdata-issues/issues/detail?id=1691") 
     */
...
//        System.out.println("downloading " + feed.getEntries().size() + " spreadsheets.");
        ...
        /* Allow to download additional spreadsheets */
        docClient.setUserToken(spreadsheetsToken.getValue());
    ...
    */
    ...
    throws IOException, ServiceException {
        ...
        System.out.println(FOLDER_FEED_URI);
    ...
    throws IOException, MalformedURLException, ServiceException {
        ...
        String exportUrl = "https://spreadsheets.google.com/feeds/download/spreadsheets/Export?key=" + docId + "&exportFormat=csv";
        ...
        int numWorksheets = getNumWorksheets(resourceId);
        //int numWorksheets = 1;
        System.out.println("Need to download "+numWorksheets+" sheets.");
        // i = number of sheets downloaded.
        // j = gid
        int i = 0;
        for (int j=0; i<numWorksheets && j < numWorksheets + 100; j++) { // max: 10 gid's above numworksheets
        	...
        	try {
                ...
                System.out.println("Downloaded: "+exportUrl + "&gid=" + j);
                if (!downloadedFiles.contains(filepath + i + ".csv")) {
                    downloadedFiles.add(filepath + i + ".csv");    
                }
                i++;
            } catch(ServiceException e) { // google's bug...
            	//System.out.println("Did not download file: "+exportUrl + "&gid=" + j);
            	//System.out.println("Error for "+j+": "+e);
            } 
        }
       	if(i == numWorksheets){
    		System.out.println("Successfully downloaded all sheets.");
       	}else {
       		System.out.println("Failed to download "+(numWorksheets-i)+" of "+numWorksheets+" sheets. ");
    	}
    }
